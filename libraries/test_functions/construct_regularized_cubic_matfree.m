function f_handle = construct_regularized_cubic_matfree(n, mat_cond, tensor_cond)
    ...
    f_handle = @(x) regularized_cubic_matfree( ...
                                              x, vec, mat_unitary, mat_diag, tensor_diag, tensor_unitary, p);
end

function [f, der1f, der2f, der3f] = regularized_cubic_matfree(x, vec, ...
                                                              mat_unitary, mat_diag, tensor_diag, tensor_unitary, p)
    % Matrix-free version of your regularized cubic test function:
    %   f(x) = vec' * x + 1/2 * x' * H * x + 1/6 * tensor[x]^3 + ||x||^p
    % where:
    %   H = mat_unitary * diag(mat_diag) * mat_unitary'
    %   tensor has CP form with (tensor_unitary, tensor_diag)
    %
    % Returns:
    %   der2f as v |-> H(x) v
    ...
    % Interaction with T via CP factors (no dense 3-tensor)
    ...
    if nargout > 3
        der3f = @(s) @(v) third_dir_apply_matfree( ...
                                                  s, v, tensor_unitary, td, x, norm_x, p);
    end
end
...
function w = third_dir_apply_matfree(s, v, tensor_unitary, td, x, norm_x, p)
