function [fun, der1f, der2f, der3f] = chebysv_rosenbrock_matfree(x)
    ...
    % Hessian-vector product (matrix-free), using your tri-diagonal structure
    if nargout > 2
    ...
    % Uses exactly the nonzeros you set in your dense tensor:
...
function w = apply_third_dir(x, s, v, rho)
    ...
    % in the original dense-3rd-derivative code.
    d = length(x);
    w = zeros(d, 1);
