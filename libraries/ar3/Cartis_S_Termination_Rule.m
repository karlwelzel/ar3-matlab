classdef Cartis_S_Termination_Rule < Termination_Rule
    % Implements the s rule termination condition from the ARC literature
    ...
    methods
        ...
        function [terminate, status] = should_terminate(obj, run)
            ...
            % numerical results" equation (7.2)
            if run.norm_g < min(obj.relative_tolerance_g, norm(run.x)) * obj.outer_run.norm_g
                terminate = true;
